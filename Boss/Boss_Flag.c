...
void Boss_flag_send(boss_flag_obj_t *p_flag_obj, boss_flags_t set_flags)
{
  ...
  {
    _flag_wait_t *p_wait = p_flag_obj->wait_list;
...
/*===========================================================================
    _   B O S S _ F L A G _ O P T _ R E C E I V E
---------------------------------------------------------------------------*/
static boss_flags_t _Boss_flag_opt_receive(boss_flag_obj_t *p_flag_obj,
                                        boss_flags_t wait_flags, boss_u16_t opt)
{
  boss_flags_t    flags;
...
/*===========================================================================
    B O S S _ F L A G _ R E C E I V E _ O R
---------------------------------------------------------------------------*/
boss_flags_t Boss_flag_receive_or(boss_flag_obj_t *p_flag_obj, boss_flags_t or_flags)
{
  return _Boss_flag_opt_receive(p_flag_obj, or_flags, __FLAG_OPT__OR);
}


/*===========================================================================
    B O S S _ F L A G _ R E C E I V E _ A N D
---------------------------------------------------------------------------*/
boss_flags_t Boss_flag_receive_and(boss_flag_obj_t *p_flag_obj, boss_flags_t and_flags)
{
  return _Boss_flag_opt_receive(p_flag_obj, and_flags, __FLAG_OPT_AND);
}
                ...
                boss_flags_t wait_flags, boss_tmr_ms_t timeout, boss_u16_t opt)
{
  ...
  flags = _Boss_flag_opt_receive(p_flag_obj, wait_flags, opt);
  if( flags == 0 )
  {
    /* 이벤트 플래그 리스트에 추가 */
    if(p_flag_obj->wait_list != _BOSS_NULL) {
    ...
    do {
      Boss_sig_clear( Boss_self(), SIG_BOSS_SUCCESS | SIG_BOSS_TIMEOUT);
      BOSS_IRQ_RESTORE_SR(irq_storage);

      if(timeout == WAIT_FOREVER) {
          _Boss_wait_sig_forever(SIG_BOSS_SUCCESS);                     /* 무한 대기 */
      } else {
          timeout = _Boss_wait_sig_timeout(SIG_BOSS_SUCCESS, timeout);  /* 타임아웃 대기  */
          BOSS_ASSERT((timeout != 0) || (Boss_self()->sigs & SIG_BOSS_TIMEOUT));
      }
    ...
    } while( (flags == 0) && ((Boss_self()->sigs & SIG_BOSS_TIMEOUT) == 0) );
...
/*===========================================================================
    B O S S _ F L A G _ W A I T _ O R
---------------------------------------------------------------------------*/
