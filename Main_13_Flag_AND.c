...
{  
  ...
  for(;;)
  {
    ...
    Boss_sleep(1000);
    PRINTF("[%s] (%d) Boss_flag_send( FLAG_02 )\n", Boss_self()->name, aa_count);
    Boss_flag_send(&test_flag_grp, FLAG_02);

    Boss_sleep(1000);
    PRINTF("[%s] (%d) Boss_flag_send( FLAG_03 )\n", Boss_self()->name, aa_count);
...
int bb_task(void *p_arg)
{
  ...
  for(;;)
  {
    boss_u16_t flags = Boss_flag_wait(&test_flag_grp, FLAG_01 | FLAG_02 | FLAG_03,
                                _FLAG_OPT_AND + _FLAG_OPT_CONSUME, 20*1000/*20초*/);
    
    if(flags != 0)
    {
      PRINTF("[%s] Boss_flag_wait(AND + CONSUME) flags = 0x%04x\n",
...
/*
        ########## 실행 결과 ##########
            ...
            [BB] Boss_flag_wait(AND + CONSUME) flags = 0x0007
             FLAG_01
             FLAG_02
             FLAG_03
